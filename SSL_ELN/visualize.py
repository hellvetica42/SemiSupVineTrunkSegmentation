...
from dataset.common import imagenet_mean, imagenet_std, colors_voc, colors_city, colors_vine
...
def visualize_image(image): # vis image itself with mean train
    ...
    img = np.transpose(original_image, (1,2,0))
    cv2.imwrite("./input.jpg", img)
    return img



def visualize_segmap(seg_map, dataset='vine', label=False):
    ...
    if dataset == 'voc':
        ...
        colors_voc_origin = torch.Tensor(colors_city)
        new_im = colors_voc_origin[target.long()].cpu().numpy()
...
def visualize_binary_mask(bin_mask): # vis image itself with mean train
    ...
    new_im = bin_color[target.long()].cpu().numpy()
    new_im = new_im.astype(np.uint8)
...
def visualize_filtered_map(cls_output, bin_mask, dataset):
    ...
    if dataset == 'voc':
        ...
        filtered_output = torch.where(bin_mask == 1, 0, 1)
        colors_voc_origin = torch.Tensor(colors_vine).cpu()

        
    new_im = colors_voc_origin[filtered_output.long()].cpu().numpy()
...
def main(args):
    ...
    print(f"{cls_output.shape=}, {img_norm.shape=}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--backbone_name', type=int, choices=[50,101], default=50)
