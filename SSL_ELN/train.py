...
from visualize import visualize_image, visualize_segmap
...
class TrainManager(object):
    ...
    def validate(self, epoch, loader):
        ...
        if standard == 0:
            ...
            with autocast():
                with torch.no_grad():
                    for idx, (image, target) in tqdm(enumerate(loader), leave=True, position=0, disable=True):
                        ...
                        if epoch >= self.warm_up_epoch:
                            ...
                            try:
                                cls_output2 = F.interpolate(output2, size=image.size()[2:], mode='bilinear', align_corners=True)
                                out_img2 = visualize_segmap(cls_output2, 'vine')
                                wandb.log({
                                    'validation/out_img2' : wandb.Image(Image.fromarray(out_img2[0]))
                                })
                            except Exception as e:
                                print(e)

                        try:
                            cls_output = F.interpolate(output, size=image.size()[2:], mode='bilinear', align_corners=True)

                            in_img = visualize_image(image)
                            out_img = visualize_segmap(cls_output, 'vine')
                            label = visualize_segmap(target, 'vine', label=True)

                            print(f"{in_img.shape=}, {out_img.shape=}, {label.shape=}")

                            wandb.log({
                                    'validation/in_img' : wandb.Image(Image.fromarray(in_img)),
                                    'validation/out_img' : wandb.Image(Image.fromarray(out_img[0])),
                                    'validation/label' : wandb.Image(Image.fromarray(label[0]))
                            })
                        except Exception as e:
                            print(e)
                ...
                wandb.log({'validation/student_net' : m_iou,
                           'validation/in_img' : wandb.Image(Image.fromarray(in_img)),
                           'validation/out_img' : wandb.Image(Image.fromarray(out_img)),
                           'validation/label' : wandb.Image(Image.fromarray(label))
                })
    ...
    def train_both(self, epoch, dataloader, iter_per_epoch):
        ...
        for batch_idx in range(iter_per_epoch):
            ...
            #print(f"{inputs_l.shape=}, {labels_l.shape=}, {inputs_ul.shape=}, {labels_ul.shape=}, {inputs_ul_aug.shape=}")
            if epoch >= self.warm_up_epoch:
    ...
    def save_ckpt(self, epoch):
        if epoch % self.args.save_ckpt == 0:
            ...
            if not os.path.isdir(pjn('checkpoints', self.save_dir)):
                os.makedirs(pjn('checkpoints', self.save_dir))
...
def main(args):
    ...
    elif args.dataset == 'vine':
        num_classes = num_classes_vine
    else:
        raise ValueError
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='vine',
